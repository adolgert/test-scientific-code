...
A unit test usually starts with some setup of variables and
parameters. This can be complicated. Then comes the moment it
calls the function-under-test with those parameters. Finally,
it checks the result. Let's say we can figure out what the result
should be, for any set of input parameters. We can talk later about
when this is possible, and when it isn't.

A decision table is a set of unit tests, one per row,
with a column for each parameter choice and a column
...
We might define equivalence classes for each
...
What would it mean to test all possible parameters?
What would it mean to test all possible failures?
What would it mean to test all possible faults?
What would it mean to test all possible expectations?


\subsubsection{Boundary values for floating point types}
...
\subsection{Equivalence classes for vector parameters}

\subsubsection{Variables, parameters, and options}
