...
\begin{lstlisting}
quadratic_root_positive <- function(a, b, c) (-b + b^2 - 4 * a * c) / (2 * a)
\end{lstlisting}
...
Floating-point types represent the range of real numbers, from a maximum
negative value to a maximum positive value, and negative and positive infinity
(\lstinline|Inf|, \lstinline|-Inf|),
and negative and positive values of zero (\lstinline|0| and \lstlinline|-0|). They also represent not-a-number,
called \nan, which is not a real number.

There are three main parts to the bit pattern that represents a floating-point
type. One bit is the sign. Then there are
...
Others are surprising, like the failure of an equality test~\citep{muller2018handbook}.
...
\begin{itemize}
    \item \emph{Invalid operation}---For example, the square root of a negative
    number, zero divided by zero, adding infinity to itself, or comparison with \nan. Results in a \nan, which means
    ``not a number.''
