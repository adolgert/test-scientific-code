...
For example, let's say our function is $2xsin(x)$. We know that
$f(\pi/2)$ is blah and $f'(\pi/2)$ is blah. Then we can test.
\begin{lstlisting}
x0 = 0.5 * pi
abs(f(x0 + x) - f(x0) * x
\end{lstlisting}
It will be a quadratic that hits zero, with higher-order terms that
are smaller.
...
Use the scaling relations, where you set up the subspace.
