...
in code, let's assess the risks that are unigue to scientific code.
...
The tests, themselves, can even be the problem when we randomly
...
chance that's \textsc{ok}. Some distributions, such as the Cauchy
...
the behavior of multiple runs of a function. For code that
doesn't have mathematical complexity, unit tests can assert
that there was some set of parameters for which the output
of the function was correct. For scientific code, we may
lack even that, but statistics, like those above and the many
more not mentioned, assert that a function
can't be too wrong.
...
functions, the function under test and some parable form
of that function, written into the unit test itself.

There are times that it's reasonable to write a test version
of a function, some second, similar way. More often, we write
a function once using its most obvious, probably slowest, algorithm.
The obvious algorithm is an excellent unit test on any refinement.

Two implementations of the same model differ based on how early
in the design they diverge from each other.
...
then they it takes more care to use one for testing of the other.
...
for a Gompertz-Makeham mortality law,
\begin{equation}
  \mu(x) = \alpha e^{\beta x} + \lambda.
\end{equation}
The function under test is a formula
...
We can check the code with a parallel implementation,
where the new version does the integrals in the equation
above. It will be very slow by comparison, but the answer should
be the same within single precision if we are working in double precision.
