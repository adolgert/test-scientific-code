...
\begin{itemize}
    \item Set scope: functional, the right answer, not performance, security, usability.
    \item Why this? If you want to understand how a thing works, study how it breaks.
\end{itemize}
...
the problem. Worst, they could be wrong numbers that are
...
\end{itemize}
While algorithm defects are immediate threats to correctness, the
ODC paper found tended to find them early in development and testing, and
tests in assignment and checking are no less harmful. That means
we get the core algorithm correct early and shoot ourselves in
the foot as we add options to the code.

There's another classification scheme for faults that asks
\emph{where} they happen, in what part of the code. For scientific computing,
we can break that down into a few pieces.
\begin{itemize}
...
We find problems in code by reading it, by testing it, or
by running it. Both reading and testing rely on our ability
to see cause and effect in code.
